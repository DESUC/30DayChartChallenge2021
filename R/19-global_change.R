...
df_gender <- read_csv("raw_data.csv") %>% 
  janitor::clean_names() %>% 
...
p <- ggplot() +
  ...
  labs(title = "Diferencias en el Índice de Brechas de Género entre el 2019 y 2020") +
  ...
  theme(axis.text = element_blank(),
        ...
        plot.title = element_text(colour = "#edf2f4", family = "Trebuchet MS"),
...
q <- df_gender_longer_rank %>% 
  ...
  labs(x = "Países de América - Posición en el ranking internacional",
       ...
       title = "
Posicionamiento en el Índice de Brechas de Género.
...
ggsave("output/19-global_change.png", width = 18, height = 14, units = "cm")
